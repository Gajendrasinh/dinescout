...
/** Thin HttpClient wrapper: prefixes the API base URL and unwraps the
 *  backend's standard `{ data, meta }` / `{ data }` success envelope. */
...
export class ApiClientService {
  ...
  getList<T>(path: string, query?: object): Observable<ApiListResult<T>> {
  ...
  delete<T>(path: string): Observable<T> {
    return this.http.delete<{ data: T }>(`${this.baseUrl}${path}`).pipe(map((res) => res.data));
  }
